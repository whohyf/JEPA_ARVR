"""D3 E3: sweep observed temporal context length at fixed oracle future tokens.

Probe input is always built from true future latents (oracle_target). Only the
observed prefix length changes, in units of temporal chunks (default grid 16).
"""
...
logger = logging.getLogger("probe_context_length_scan")
...
def _probe_tokens(observed_last: torch.Tensor, oracle_target: torch.Tensor, context_chunks: int, spatial: int):
    if context_chunks <= 0:
        return oracle_target
    prefix = observed_last[:, -context_chunks * spatial :, :]
    return torch.cat([prefix, oracle_target], dim=1)


def _method_label(context_chunks: int) -> str:
    return "oracle_target_only" if context_chunks <= 0 else f"oracle_ctx{context_chunks}"
...
def run(args):
    ...
    encoder, _predictor = _load_encoder_predictor(cfg, device)
    ...
    logger.info(
        "E3 context scan horizon=%.3fs chunks=%s spatial=%d samples=%d",
        args.horizon,
        context_chunks_list,
        spatial,
        len(ds),
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        with torch.cuda.amp.autocast(dtype=torch.bfloat16, enabled=use_bfloat16):
            ...
            for context_chunks in context_chunks_list:
                ...
                tokens = _probe_tokens(observed_last, oracle_target, context_chunks, spatial)
    ...
    for row in native_summary_rows:
        method = row["method"]
        chunks = 0 if method == "oracle_target_only" else int(method.removeprefix("oracle_ctx"))
        curve_rows.append(
            {
    ...
    summary = {
        "experiment": "D3-E3-probe-context-length-scan",
    ...
    logger.info("Wrote E3 context scan to %s", args.out_dir)
