...
# ViT-L @256 fp32 fulltrain, 10s AR sliding window + gaze/pose + encoder LoRA.
# Val fixed @10s; train samples anticipation in [8, 10]s to reduce overfitting.
# Slurm regenerates yaml — verify .out has "Applied AR10s sliding config" + config check.
#
# NOTE: AR10s rollout OOMs at bs8 on single H100 (~78GB). Use
# submit_b11_singleprobe_ar10s_sliding_gaze_pose_fulltrain.sh (bs4 w2) instead.
...
TAG="${LORA_TAG:-hdepic-singleprobe-ar10s-vitl-fp32-bs8-noac-10ep-w${WORKERS}-tr8-10}"
...
echo "[submit-vitl-ar10s-fulltrain] tag=${TAG}"
echo "[submit-vitl-ar10s-fulltrain] val=10s train=[8,10]s rollout/final_window fp32 bs8 no-AC workers=${WORKERS}"
