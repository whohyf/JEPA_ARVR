...
# VLM baseline under the current HD-EPIC protocol:
# - p01_fixed split and PhD-reference class/timing defaults
# - 1s action anticipation
# - RGB-only observed clip
# - frozen Hugging Face VLM/vision encoder replaces V-JEPA2 encoder+predictor
# - attentive probe is trained from scratch because VLM feature dim differs from V-JEPA
...
TAG="${LORA_TAG:-hdepic-vlm-${VLM_MODEL_CLASS}-1s-p01fixed-rgbonly-probeonly-${VLM_NUM_FRAMES}f4s-${VLM_TOKEN_MODE}-bs${BATCH_SIZE}-w${WORKERS}}"
...
echo "[submit-vlm-1s-p01fixed-rgbonly-probeonly] tag=${TAG}"
echo "[submit-vlm-1s-p01fixed-rgbonly-probeonly] code_root=${PROJECT_ROOT} shared_root=${SHARED_PROJECT_ROOT}"
echo "[submit-vlm-1s-p01fixed-rgbonly-probeonly] model=${VLM_MODEL_ID} class=${VLM_MODEL_CLASS} frames=${VLM_NUM_FRAMES} token_mode=${VLM_TOKEN_MODE} batch=${BATCH_SIZE}"
echo "[submit-vlm-1s-p01fixed-rgbonly-probeonly] gpu_pulse_iters=${VLM_GPU_PULSE_ITERS} gpu_pulse_size=${VLM_GPU_PULSE_SIZE}"
echo "[submit-vlm-1s-p01fixed-rgbonly-probeonly] split=p01_fixed; MODEL_FAMILY=vlm; GAZE_MODE=none; encoder/predictor LoRA disabled"
echo "[submit-vlm-1s-p01fixed-rgbonly-probeonly] partition=${PARTITION} gres=${GRES} mem=${MEM} cpus-per-task=${CPUS_PER_TASK} workers=${WORKERS}/${VAL_WORKERS}"
