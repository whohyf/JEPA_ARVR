...
# Unattended watchdog for the VLM (CLIP-L/14) p01_fixed RGB-only probe-only
# baseline: monitor the job, and relaunch (with RESUME_CHECKPOINT=1) on
# low-GPU-utilization cancel or OOM. Mirrors
# watch_and_relaunch_p01fixed_rgbonly_probeonly.sh, which handles the
# structurally identical issue on the frozen-V-JEPA2-encoder RGB-only
# baseline.
#
# Usage:
#   bash scripts/watch_and_relaunch_vlm_baseline_1s_p01fixed_rgbonly_probeonly.sh --job-id 11174681
#
# Logs: logs/watchdog_vlm_baseline_1s_p01fixed_rgbonly_probeonly_<first_jobid>.log

PROJECT_ROOT="${PROJECT_ROOT:-/scratch/yh6416/VJEPA2-EXP}"
SUBMIT_SCRIPT="${SUBMIT_SCRIPT:-${PROJECT_ROOT}/.worktrees/vlm-baseline-1s/scripts/submit_b11_vlm_baseline_1s_p01fixed_rgbonly_probeonly_llama32vision_resumable.sh}"
job_id=""
max_relaunch=8
poll_sec=120
...
first_job_id="${job_id}"
log_file="${PROJECT_ROOT}/logs/watchdog_vlm_baseline_1s_p01fixed_rgbonly_probeonly_llama32vision_${first_job_id}.log"
state_file="${PROJECT_ROOT}/logs/watchdog_vlm_baseline_1s_p01fixed_rgbonly_probeonly_llama32vision_${first_job_id}.state"
